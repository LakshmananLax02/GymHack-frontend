...
// ─────────────────────────────────────────────────────────────────────────────
const RAZORPAY_KEY_ID    = 'rzp_test_SfeLlF57PYZ7bE'; // your key
...
// Inside your PaymentScreen Component
const launch = async () => {
  setPaying(true);
  setError('');
  ...
  try {
    ...
    // We send the cart and form data so the server can calculate the price securely
    const response = await fetch('http://localhost:5000/api/payment/create-order', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        amount: total, // Still sent for reference, but backend should re-verify
    ...
    // STEP 2: Configure Razorpay Options using the ID from your DB/Razorpay
    const options = {
      key: orderData.key, // Your Test Key ID sent from backend
      amount: orderData.amount,
      currency: "INR",
      name: "GYM HACK",
      description: "Premium Supplements Purchase",
      order_id: orderData.order_id, // The ID generated by your backend/Razorpay
      prefill: {
    ...
    // STEP 4: Open the Checkout Modal
